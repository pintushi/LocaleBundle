...
class DebugCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('pintushi:entity-config:debug')
    ...
    protected function dumpEntityList(OutputInterface $output)
    {
        ...
        $rows = $em->getConnection()->fetchAll(
            'SELECT class_name, mode FROM pintushi_entity_config ORDER BY class_name'
        );
        foreach ($rows as $row) {
    ...
    protected function dumpEntityListFromCache(OutputInterface $output, $scope)
    {
        ...
        $cm = $this->getContainer()->get('pintushi_entity_config.config_manager');
    ...
    protected function dumpFieldList(OutputInterface $output, $className)
    {
        ...
        $rows = $em->getConnection()->fetchAll(
            'SELECT fc.field_name, fc.type, fc.mode FROM pintushi_entity_config ec'
            . ' INNER JOIN pintushi_entity_config_field fc ON fc.entity_id = ec.id'
            . ' WHERE ec.class_name = ?'
    ...
    protected function dumpNonConfigRef(OutputInterface $output, $className = null)
    {
        ...
        if (empty($className)) {
            $rows       = $em->getConnection()->fetchAll(
                'SELECT class_name FROM pintushi_entity_config ORDER BY class_name'
            );
            $classNames = [];
            foreach ($rows as $row) {
            ...
            foreach ($assocNames as $assocName) {
                ...
                if (!isset($classNames[$targetClass])) {
                    ...
                    $fieldInfo = $em->getConnection()->fetchAll(
                        'SELECT fc.type FROM pintushi_entity_config ec'
                        . ' INNER JOIN pintushi_entity_config_field fc ON fc.entity_id = ec.id'
    ...
    protected function dumpEntityConfig(OutputInterface $output, $className, $scope = null, $attrName = null)
    {
        ...
        $rows = $connection->fetchAll(
            'SELECT * FROM pintushi_entity_config WHERE class_name = ?',
            [$className],
            ['string']
        );
        foreach ($rows as $row) {
    ...
    protected function dumpEntityConfigFromCache(OutputInterface $output, $className, $scope, $attrName = null)
    {
        ...
        $cp = $this->getContainer()->get('pintushi_entity_config.config_manager')->getProvider($scope);
    ...
    protected function dumpFieldConfig(OutputInterface $output, $className, $fieldName, $scope = null, $attrName = null)
    {
        ...
        $rows = $connection->fetchAll(
            'SELECT ec.class_name, fc.* FROM pintushi_entity_config ec'
            . ' INNER JOIN pintushi_entity_config_field fc ON fc.entity_id = ec.id'
            . ' WHERE ec.class_name = ? AND fc.field_name = ?',
            [$className, $fieldName],
            ['string', 'string']
    ...
    protected function removeEntityConfigScopeOrAttribute(
        ...
        foreach ($rows as $row) {
            ...
            $connection->executeUpdate(
                'UPDATE pintushi_entity_config SET data = :data WHERE id = :id',
                ['data' => $data, 'id' => $row['id']],
    ...
    protected function removeFieldConfigScopeOrAttribute(
        ...
        $rows = $connection->fetchAll(
            'SELECT fc.* FROM pintushi_entity_config ec'
            . ' INNER JOIN pintushi_entity_config_field fc ON fc.entity_id = ec.id'
            . ' WHERE ec.class_name = ? AND fc.field_name = ?',
            [$className, $fieldName],
            ['string', 'string']
        ...
        foreach ($rows as $row) {
            ...
            $connection->executeUpdate(
                'UPDATE pintushi_entity_config_field SET data = :data WHERE id = :id',
                ['data' => $data, 'id' => $row['id']],
                ['data' => 'array', 'id' => 'integer']
            );
        }
