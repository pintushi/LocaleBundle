...
namespace Pintushi\Bundle\ConfigBundle\DependencyInjection\SystemConfiguration;
...
class ProcessorDecorator
{
    ...
    const SECTION_TREE_ROOT = 'section_tree';
    ...
    public function process(array $data)
    {
        ...
        // validate API tree
        if (isset($result[self::SECTION_TREE_ROOT])) {
            foreach ($result[self::SECTION_TREE_ROOT] as $key => &$data) {
    ...
    protected function getEmptyFinalArray()
    {
        $result = array(
            self::ROOT => array_fill_keys(
                array(self::GROUPS_NODE, self::FIELDS_ROOT, self::TREE_ROOT, self::SECTION_TREE_ROOT),
                array()
            )
        );
    ...
    protected function getConfigurationTree()
    {
        ...
        $tree->root(self::ROOT)
            ->children()
                ...
                ->variableNode(self::SECTION_TREE_ROOT)->end()
            ->end();
