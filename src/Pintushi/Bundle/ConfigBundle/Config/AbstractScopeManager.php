...
use Pintushi\Bundle\ConfigBundle\Event\ConfigManagerScopeIdUpdateEvent;
use Symfony\Component\EventDispatcher\EventDispatcherInterface;
use Pintushi\Bundle\ConfigBundle\Repository\ConfigRepository;
...
abstract class AbstractScopeManager
{
    ...
    public function save($settings, $scopeIdentifier = null)
    {
        ...
        $em = $this->doctrine->getManagerForClass('Pintushi\Bundle\ConfigBundle\Entity\Config');

        /** @var Config $config */
        $config = $this->configRepository->findByEntity($entity, $entityId);
        if (null === $config) {
            $config = new Config();
            $config
    ...
    {
        $config = $this->configRepository->findByEntity($this->getScopedEntityName(), $entityId);
