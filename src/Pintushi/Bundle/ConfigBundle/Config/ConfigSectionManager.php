...
use Pintushi\Bundle\ConfigBundle\Provider\ProviderInterface;
...
class ConfigSectionManager
{
    ...
    public function getSections()
    {
        ...
        $tree = $this->configProvider->getSectionTree();
    ...
    public function hasSection($path)
    {
        $section = null;
        try {
            $section = $this->configProvider->getSectionTree($path);
    ...
    public function getData($path, $scope = 'user')
    {
        ...
        $variables = $this->configProvider->getSectionTree($path)->getVariables(true);
        $result    = [];
    ...
    {
        $variable = $this->configProvider->getSectionTree($path)->getVariable($key);
        if (null === $variable) {
            return null;
        }
